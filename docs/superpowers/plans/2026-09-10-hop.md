...
- Module path is `github.com/thailoc-dev/hop`. Go 1.25.0, pinned in `.mise.toml`.
...
```bash
go mod init github.com/thailoc-dev/hop
...
### Task 19: Shell completion

**Files:**
- Create: `internal/sshconfig/sshconfig.go`, `internal/sshconfig/testdata/config`, `internal/sshconfig/testdata/included`
- Create: `internal/complete/docker.go`, `internal/complete/cache.go`, `internal/complete/complete.go`
- Create: `cmd/hop/completion.go`
- Modify: `internal/hopfs/paths.go` — add `CacheDir`
- Modify: `internal/sshexec/fake.go` — honour `ctx` in `Run`, add `SetRunDelay`
- Modify: `cmd/hop/tunnel.go`, `cmd/hop/down.go`, `cmd/hop/logs.go`, `cmd/hop/restart.go` — attach completion functions
- Modify: `README.md` — document installing completions
- Test: `internal/sshconfig/sshconfig_test.go`, `internal/complete/docker_test.go`, `internal/complete/cache_test.go`, `internal/complete/complete_test.go`, `cmd/hop/completion_test.go`

**Interfaces:**
- Consumes: `sshexec.Executor` (Task 5); `hopfs.Paths` (Task 2); `control.Dial`, `OpList` (Task 10); `tunnel.Status` (Task 8).
- Produces:
  - `sshconfig.Hosts(path string) ([]string, error)` — alias names, `Include` followed, wildcards excluded
  - `complete.Container{Name string; Ports []int}`; `complete.ParsePS(stdout string) []Container`; `complete.ContainerPorts(portsField string) []int`
  - `complete.Cache{Dir string; TTL time.Duration}` with `Get(host string) ([]Container, bool)` and `Put(host string, cs []Container) error` and `GetStale(host string) ([]Container, bool)`
  - `complete.Containers(ctx context.Context, ex sshexec.Executor, cache *Cache, host string) []Container`
  - `complete.Timeout` (`300 * time.Millisecond`) and `complete.CacheTTL` (`60 * time.Second`)
  - in `cmd/hop`: `completeHosts(toComplete string) []string`, `completeTunnelArgs(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective)`, `completeLocalPorts(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective)`

**The hard rule for this task:** pressing Tab must never hang the shell. Every network-backed completion runs under a 300 ms context, and on expiry falls back to a cached answer — even an expired one — or returns nothing. A wrong-but-instant completion is recoverable; a frozen terminal is not.

`~/.ssh/config` is read, never written. `sshconfig.Hosts` opens it read-only and hop has no code path that writes to it.

- [ ] **Step 1: Spike — does Cobra merge subcommands with a positional completer?**

The design assumes that a root command owning both subcommands and a `ValidArgsFunction` offers *both* at argument position 0. Verify it rather than assume it. Write this throwaway file, run it, delete it:

```go
// /tmp/cobraspike/main.go
package main
...
func main() {
	root := &cobra.Command{
		...
		ValidArgsFunction: func(_ *cobra.Command, args []string, _ string) ([]string, cobra.ShellCompDirective) {
			if len(args) == 0 {
				return []string{"host-alpha", "host-beta"}, cobra.ShellCompDirectiveNoFileComp
			}
			return nil, cobra.ShellCompDirectiveNoFileComp
		},
	}
	root.AddCommand(&cobra.Command{Use: "ls", Run: func(*cobra.Command, []string) {}})
...
```bash
mkdir -p /tmp/cobraspike && cd /tmp/cobraspike
go mod init spike && go get github.com/spf13/cobra@latest
go run . __complete ""
```

**Decision:**
- If the output contains **both** `ls` and `host-alpha`, Cobra merges. Proceed with the task exactly as written.
- If it contains **only** `ls`, Cobra does not merge. Then: attach `ValidArgsFunction` to the `hop tunnel` subcommand only, note in the README that host completion requires the explicit `hop tunnel <TAB>` form, and change `TestBareFormCompletesHostsAndSubcommands` in Step 2 to assert against `newTunnelCmd()` instead of the root command. Everything else in this task is unaffected.

Record the answer in the commit message. Then `rm -rf /tmp/cobraspike`.
...
```
# internal/sshconfig/testdata/config
Include included

Host example-backend-dev
    HostName 10.0.0.1
    User deploy
...
```
# internal/sshconfig/testdata/included
Host included-host
    HostName 10.0.0.9
```

```go
// internal/complete/docker_test.go
package complete
...
```go
// internal/complete/cache_test.go
package complete
...
func TestCompletionScriptGenerates(t *testing.T) {
	home := t.TempDir()
	out, err := runCmd(t, home, "completion", "zsh")
	if err != nil {
...
func TestEnvFlagCompletionOffersTheThreeLabels(t *testing.T) {
	home := t.TempDir()
	out, err := runCmd(t, home, "__complete", "--env", "")
	if err != nil {
...
var _ = net.Listen // serveFakeDaemon in lifecycle_test.go supplies the listener
```

Add `"strings"` to that file's imports and delete the trailing `var _ = net.Listen` line along with the `"net"` import — they are noted here only so the helper's origin is obvious.

- [ ] **Step 3: Run tests to verify they fail**

Run: `go test ./internal/sshconfig/ ./internal/complete/ ./cmd/hop/ -run 'TestHosts|TestContainer|TestParsePS|TestCache|TestGetStale|TestComplete|TestEnvFlag' -v`
Expected: FAIL — `undefined: Hosts`, `undefined: ParsePS`, `undefined: Cache`, `undefined: completeHosts`.
...
Add `CacheDir` to `internal/hopfs/paths.go` — a field on `Paths`, set in `New` to `filepath.Join(root, "cache")`, and included in `EnsureDirs`'s loop.

Make the fake honour context and delay, in `internal/sshexec/fake.go`:
...
adding a `runDelay time.Duration` field and the `time` import, and replacing the existing `Run`.

```go
// cmd/hop/completion.go
package main
...
Wire the completers up. In `newRootCmd`, on the root command and on `newTunnelCmd()`:
...
```go
	cmd.ValidArgsFunction = completeTunnelArgs   // inside newTunnelCmd
```

In `addTunnelFlags`, after registering `--env`:
...
And on each of `newDownCmd()`, `newLogsCmd()`, `newRestartCmd()`:

```go
	cmd.ValidArgsFunction = completeLocalPorts
```

Finally add `"completion"` to `reservedWords` in `cmd/hop/tunnel.go`, and delete the `name == "completion"` skip from `TestReservedWordsCoverEverySubcommand` so the guard covers it.

- [ ] **Step 5: Run tests to verify they pass**

Run: `go test ./... -short -race && make check`
Expected: PASS everywhere.

Then confirm it works in your actual shell:

```bash
make install
echo 'source <(hop completion zsh)' >> ~/.zshrc   # or write to a file in $fpath
exec zsh

hop <TAB>                    # ssh config hosts, plus subcommands
hop <your-host> <TAB>        # container names
hop <your-host> <container> <TAB>   # remote ports
hop down <TAB>               # running tunnels, with descriptions
```

Then prove the timeout holds, which is the point of the whole task:

```bash
# Point at a host that will not answer, and press Tab.
printf 'Host unreachable-test\n    HostName 203.0.113.1\n' >> ~/.ssh/config
hop unreachable-test <TAB>   # must return within a blink, offering nothing
```

Remove that stanza afterwards.

- [ ] **Step 6: Document and commit**

Add to `README.md` under a new `## Completion` heading:
...
returns immediately with nothing rather than hanging the shell. `~/.ssh/config`
is only ever read.
````

```bash
git add internal/sshconfig/ internal/complete/ internal/hopfs/ internal/sshexec/ cmd/hop/ README.md
git commit -m "feat(complete): host, container and port completion with a hard timeout

Cobra spike result: <merges subcommands with ValidArgsFunction / does not>."
```
...
- [ ] Tab completion offers hosts, containers, remote ports, and running local ports.
- [ ] Pressing Tab against an unreachable host returns within a blink, offering nothing.
- [ ] `~/.ssh/config` is unmodified — `hop` only ever opens it for reading.
